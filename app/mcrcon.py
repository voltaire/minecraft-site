...
class MCRcon:
    ...
    def send_real(self, out_type, out_data):
        ...
        while ready:
            # Receive an item
            tmp_len, tmp_req_id, tmp_type = struct.unpack(
                '<iii', self.s.recv(12))
            # -8 because we've already read the 2nd and 3rd integer fields
            tmp_data = self.s.recv(tmp_len-8)
            # Error checking
            ...
            m = re.match(
                '^Error executing: %s \((.*)\)$' % re.escape(out_data),
                tmp_data)
            if m:
